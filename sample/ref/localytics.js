...
extend(LocalyticsPrototype, {
    ...
    storage: function() {
        var instance = this,
            prefix = '_loc_' + ((instance.namespace) ? instance.namespace + '_' : '');
...
extend(LocalyticsPrototype, {
    _url: function(data) {
        var params = each(data, function(el, i) {
            data[i] = stringify(el);
        }).join(escape('\n'));

        // Note: we include callback, but using the image upload method it does't do anything
        return [uploadURL, this.options.appKey, '/uploads/image.gif?client_date=', time(), '&callback=z&data=', params].join('');
    },
    _upload: function(data) {
        ...
        // TODO: Should there be an upload queue to batch uploads?
        var img = doc.createElement('img');
            img.src = this._url([this._header(), data]);
    },
...
},{"./cookie":3,"./localStorage":4,"json3":2}],2:[function(_dereq_,module,exports){
(function (global){
